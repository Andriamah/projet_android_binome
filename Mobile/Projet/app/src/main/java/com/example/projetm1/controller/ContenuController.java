...
public class ContenuController {
    ...
    public void getContenuById(int id_contenu, final ContenuController.GetContenuByIdCallback callback) {
        // Construire l'URL de l'API pour récupérer les détails du client avec l'ID donné
        String apiUrl = ApiConfig.BASE_URL + "/contenu/detail/" + id_contenu;
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                // Gérer les erreurs de la requête
                e.printStackTrace();
                callback.onGetContenuByIdFailure("Erreur de connexion");
            }
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                try {
                    ...
                    Gson gson = new Gson();
                    Contenu_client_zone contenu = gson.fromJson(jsonResponse, Contenu_client_zone.class);

                    // Appeler la méthode de succès du callback avec le client récupéré
                    callback.onGetContenuByIdSuccess(contenu);
                } catch (Exception e) {
                    e.printStackTrace();
                    callback.onGetContenuByIdFailure("Erreur de traitement de la réponse du serveur");
                }
            }
        });
    }

    // Interface pour le callback de récupération du contenni par ID
    public interface GetContenuByIdCallback {
        void onGetContenuByIdSuccess(Contenu_client_zone contenu);

        void onGetContenuByIdFailure(String errorMessage);
    }
}
